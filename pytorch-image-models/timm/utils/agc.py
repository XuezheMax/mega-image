...
def adaptive_clip_grad2(parameters, clip_factor=0.01, eps=1e-6, norm_type=2.0):
    if isinstance(parameters, torch.Tensor):
        parameters = [parameters]
    for p in parameters:
        if p.grad is None:
        ...
        p_data = p.detach()
        g_data = p.grad.detach()
        grad_norm = torch.norm(g_data, p=norm_type)
        max_norm = torch.norm(p_data, p=norm_type).clamp_(min=eps).mul_(clip_factor)
