...
def add_weight_decay(model, weight_decay, skip_list=()):
    ...
    for name, param in model.named_parameters():
        ...
        if name.endswith(".bias") or (len(param.shape) == 1 and name.endswith(".weight")) or name in skip_list:
...
def create_optimizer(args, model, filter_bias_bn=True):
...
def create_optimizer_v2(
    ...
    Args:
        ...
        filter_bias_bn: filter out bias and bn weight and other 1d params from weight decay
    ...
    if isinstance(model_or_params, nn.Module):
        ...
        if weight_decay and filter_bias_bn:
            skip = {}
